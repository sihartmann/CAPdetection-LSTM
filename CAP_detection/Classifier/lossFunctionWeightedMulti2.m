classdef lossFunctionWeightedMulti2 < nnet.layer.ClassificationLayer
    ...
    methods
        function layer = lossFunctionWeightedMulti2(w1,w2,w3,name)           
        ...
        function loss = forwardLoss(layer, Y, T)
            ...
            loss = -1/length(Y)*(layer.w1*sum(T(1,:).*log(Y(1,:))) + layer.w2*sum((T(2,:)).*log(Y(2,:))) + layer.w3*sum((T(3,:)).*log(Y(3,:))));
        end
